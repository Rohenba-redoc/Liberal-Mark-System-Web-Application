      ...
      <form id="streamsForm" class="mb-4">
        ...
        <table class="table table-bordered" id="streamsTable">
          <thead>
            <tr>
              <th>Sl.No</th>
              <th>Stream Title<span class="text-red">*</span></th>
              <th>Action</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td class="slno">1</td>
              <td><input type="text" name="stream_titles[]" class="form-control"></td>
        ...
        <a href="stream.php" class="btn w-100 btn-link text-body-secondary mt-2">
          Cancel
        </a>
...
<script>
  document.addEventListener("DOMContentLoaded", function() {
    ...
    addRowButton.addEventListener("click", function() {
      ...
      newCell1.innerHTML = '<input type="text" name="stream_titles[]" class="form-control">';
    ...
    form.addEventListener("submit", function(event) {
      ...
      const streamTitles = formData.getAll("stream_titles[]");

      const streams = streamTitles.map(title => ({
        title
      }));

      fetch('../functions/add_stream.php', {
        method: 'POST',
